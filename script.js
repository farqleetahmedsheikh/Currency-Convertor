...
try {
  for (let select in selects) { // Accessing  each <select> element and adding options
    for (let country in countryList) { // Accessing  each country from list
      ...
      if (selects[select].name === "from" && country === "PKR") { // selecting pkr by default  as the base currency
        newOption.selected = "selected";
      } else if (selects[select].name === "to" && country === "USD") {  // selecting  usd as default currency when converting to
        newOption.selected = "selected";
      }
...
exchangeBtn.addEventListener("click", async (evt) => {  // async function to get  data when clicked from api of selected currency 
  ...
  const URL = await fetch(
    `https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/${fromSelect.value.toLowerCase()}.json`
  ); // fecting data from api url in json format
  let data = await URL.json();  // converting api data into readable format 
  let rate = data[`${fromSelect.value.toLowerCase()}`];  // selecting the country rate from the data
  let rate1= rate[`${toSelect.value.toLowerCase()}`];
  // console.log(rate1);
  let finalAmount = amountVal * rate1;
...
// https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/${fromSelect.value.toLowerCase()}/${toSelect.value.toLowerCase()}.json

// data[toSelect.value.toLowerCase()]